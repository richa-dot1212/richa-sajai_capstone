...
- **What didn't work on the first try:** my first version of the proxy set the corrected `issuer` field to `${PROXY_BASE}/` (trailing slash) while the synthesized protected-resource document's `authorization_servers` entry had no trailing slash — `mcp-remote` compared the two and threw the same `IssuerMismatchError`, just against my own proxy instead of the real server. Fixed by making both use the exact same string (no trailing slash), confirmed by re-running: `mcp-remote` then got a real, valid Swiggy authorization URL and reached the "waiting for login" state — the bug is fixed as far as this session can verify. Completing the actual OAuth login still requires the user's own Swiggy account in an interactive browser session, which isn't possible from here — documented as the concrete first step for PR 2.
