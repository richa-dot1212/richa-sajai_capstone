...
## 2026-09-24 -- Genuinely agentic per-ingredient reasoning loop, real overrides (new branch: agentic-per-ingredient-reasoning)

- **Time spent:** ~2.5 hours (3 parallel Explore agents to map the exact current architecture before touching anything, plan-mode design + approval, implementation across workflow/decision/Skill/server/frontend, and real functional testing against actual code paths)
- **Rough tokens used:** ~55-65K
- **Context:** the 2-LLM-call design (1 recipe parse + 1 batched role/substitute call for every missing ingredient at once) was built specifically to survive Gemini's 20-requests/day free-tier quota. Now on Groq, that constraint no longer applies, and the user explicitly asked for the design priority to shift to correct, defensible agentic reasoning -- a real per-ingredient perceive-reason-act-observe loop, with an explicit essential-ingredient concept (a chicken recipe missing chicken should never get an invented unrelated substitute like tofu, unless the user actually asked for a vegetarian transformation).
- **What shipped:**
  - **agent/workflow.js**: replaced the single batched ROLE_BATCH_SCHEMA LLM call (made once, before any Instamart data existed) with one LLM call per missing ingredient, made inside the existing per-ingredient loop right before that ingredient's Instamart search -- a real interleaved reason-act-observe-reason-again-act-observe loop, not one upfront prompt deciding everything. New schema (INGREDIENT_REASONING_SCHEMA) adds isEssential (true = do not invent an unrelated substitute) and a plain-language roleExplanation field for the end user, alongside the existing role/compatibility fields. The prompt now also passes input.requestedChanges into this reasoning step -- a real, previously-existing gap: the user's dietary/transformation requests were collected from the form but never actually reached this reasoning step, so there was no way for the model to know a vegetarian substitute was actually wanted. When an ingredient is essential with no substitute proposed, the substitute Instamart search is skipped entirely (saves a call, avoids searching for a nonsense product). agent/decision.js needed zero logic changes -- confirmed by reading it closely that its existing branches already produce exactly the 5 behaviors the user specified, once the upstream judgment correctly reports "essential, no substitute" instead of always inventing one. Call count changes from a flat 2 to 1+N (one per missing ingredient) -- an explicit, intended trade-off per the user's own instruction that correctness now outweighs call minimization.
  - **.claude/skills/recipe-budget-agent/SKILL.md**: added a new section 2 teaching the essential/defining-vs-substitutable classification as its own explicit step (with the dietary-transformation carve-out), and added guidance that the role explanation should be plain language for the end user, not just an internal category label. Renumbered the remaining sections (old 2-9 -> new 3-10); their content is otherwise unchanged.
  - **server.js**: exported buildSummary from workflow.js (was private) so it can be reused after an override instead of duplicated. The in-memory job object now retains state (full decision records, including both the original and substitute product data), addressId, and sessionId after a run completes. New route, POST /api/adapt/:jobId/override ({ ingredient, choice: 'original'|'substitute' }): looks up the matching decision, calls the real Instamart update_cart for the chosen product (attempting to remove the previously-added item first, if there was a different one), updates that decision record in place, recomputes spentSoFar deterministically, and returns a freshly rebuilt summary -- does not re-run the workflow.
  - **public/app.js / style.css**: buildSummary gains an additive ingredientDecisions array (role, explanation, substitute info, both real prices, the decision, and the reasoning) -- the existing ingredients[].note / substitutions / cartItems / unresolved fields are untouched. renderIngredientsTab gains a new "decision card" per missing ingredient (matching the user's example format: why it's used, substitution explanation, price comparison, the automatic decision, and a real override button for whichever option wasn't chosen). A single delegated click listener on #summary handles override clicks (required, since #summary's contents are fully replaced by innerHTML on every render) -- it POSTs to the new route and re-renders from the returned summary, restoring whichever tab was open.
- **What didn't work on the first try, and what was actually tested (not assumed):**
  1. Address resolution (get_addresses) happens unconditionally before the reasoning loop and throws immediately when not logged into Swiggy -- pre-existing behavior, not introduced here, but it meant the new per-ingredient loop couldn't be reached at all without a real login (which needs an interactive browser this environment doesn't have, same constraint documented throughout this project). Tested the real code by stubbing only the Instamart network boundary (InstamartClient.prototype.init/callTool) in a throwaway script, so the actual reasoning/decision/record-building logic in workflow.js ran for real against real Groq calls -- not simulated.
  2. First test stub had the wrong product shape (variation.offerPrice instead of the real variation.price.offerPrice that extractTopProduct actually reads) -- caught immediately from a null price showing up everywhere in the output, fixed the stub, confirmed it was a test-double bug and not a bug in the real code.
  3. Hit a real Groq 429 (8000 tokens/minute) rate limit from running several test scenarios back-to-back within the same minute -- an expected, honest consequence of 1+N calls per run costing more TPM than the old flat 2, especially under rapid repeated testing. Genuinely useful outcome: it exercised the new fallback path for a failed reasoning call, which correctly defaults to "essential, no substitute" (the safest possible default) and lets the run complete rather than crashing.
  4. Found and fixed a real bug during override testing: the decision record's human-readable reasoning text wasn't being refreshed after an override, so it would keep describing the original automatic pick even after decision had changed to reflect the user's override -- caught by actually inspecting the returned summary after a simulated override, not assumed to work.
  5. Verified, against real (stubbed-Instamart, real-LLM) runs: an essential ingredient (chicken) with no dietary change requested gets no substitute proposed at all and is reported unresolved with a real "add anyway" override when unaffordable; the same ingredient does get a real substitute (tofu) proposed when "make it vegetarian" is in requestedChanges; a non-essential ingredient (butter) gets both real prices compared and the cheaper compatible option auto-added; and a case where neither option fits the budget reports the conflict with both override options available. Also verified the override route's logic end-to-end against a real completed run's state: the real update_cart calls happen in the right order (remove-then-add), the decision record and spentSoFar update correctly, and a deliberately over-budget override (user explicitly overriding an unaffordable essential ingredient) is allowed through, as it should be.
- **Not yet tested:** the actual browser UI (decision cards' visual layout, click-to-override flow through the real DOM) and a real end-to-end run through an actual logged-in Swiggy account -- both require a real interactive login this environment cannot complete. Everything else was verified against real code execution, not assumed.

## 2026-09-24 -- Consolidate unresolved-ingredient UI into one yellow box (same branch)

- **Time spent:** ~25 min
- **Rough tokens used:** ~7-8K
- **What shipped:** user tried a real run and found the unresolved-ingredient presentation split awkwardly across three places -- a separate "Heads up" warning banner, then a per-ingredient reasoning card mixed in among the successfully-resolved ones, each with its own override button. Consolidated: `renderDecisionCards` (left column) now only renders ingredients that were actually bought or substituted; a new `renderUnresolvedBox` renders one solid-yellow box (`--gingham-yellow`, matching the header band's yellow at full opacity rather than the diluted pattern) in the right column below the Instamart cart note, holding every unresolved ingredient together -- each one's "why it's used" and "why it couldn't be added" combined into one paragraph, with its override button(s) directly below. Override buttons in this box use a new `--swiggy-orange` (Swiggy's actual brand orange, `#fc8019`) instead of the site's navy accent, as a deliberate visual cue distinct from the rest of the UI: this action means "add to Instamart," not a general site action.
- **What didn't work on the first try:** the delegated click handler looked for a `.decision-card` ancestor to disable a button's siblings together while a request is in flight -- that lookup didn't match the new yellow box's `.unresolved-box__item` structure, which would have let both "add anyway" buttons for the same ingredient stay clickable simultaneously during an in-flight request. Caught by reviewing the handler against the new markup before shipping (not caught by a live click, since no live Swiggy login exists here to test it through) and widened the selector to match either container.
- Cleaned up now-dead CSS (`.decision-card--unresolved`, `.cart-note--warning`) left behind by this restructure rather than leaving unused rules in place.

## 2026-09-24 -- Remove resolved-ingredient reasoning boxes, lighten yellow, white/orange buttons (same branch)

- **Time spent:** ~15 min
- **Rough tokens used:** ~5-6K
- **What shipped:** per feedback that seeing an ingredient already sitting in the Instamart cart note was confirmation enough -- the separate reasoning card per resolved ingredient (role explanation, substitution reasoning, price comparison, "instead" override button) was redundant once the cart note already shows it added. Removed `renderDecisionCards` and its CSS entirely; the left column is back to just the plain ingredient list. The yellow "Heads up" box (still showing every unresolved ingredient's reasoning + override buttons, since those genuinely need a decision) stays, with two changes: `--gingham-yellow` lightened from the raw gingham stripe colour (`#f7d154`, read as too saturated/bold) to a paler butter-yellow (`#f6e6ab`) closer to the gingham pattern's actual overall look; and its "add anyway" buttons flip from solid Swiggy-orange to white with an orange border/text, orange fill only on hover. Also renamed the cart note's heading from "From the Instamart cart" to "Added to the Instamart cart".
- **What didn't work on the first try:** nothing broke -- removing the decision-cards feature left one stray reference in the delegated click handler's button-group selector (`.decision-card, .unresolved-box__item`, from the previous round); cleaned it up to just the selector that still matches anything, rather than leaving a harmless-but-dead class name in a live selector.

## 2026-09-24 -- Fix real bug: automatic cart-adds weren't reaching the actual Instamart cart (same branch)

- **Time spent:** ~20 min
- **Rough tokens used:** ~6-7K
- **What shipped:** user did the first real end-to-end test against an actual logged-in Swiggy account (something this environment can't do, since it needs an interactive browser) and found a genuine bug this branch's earlier stubbed-Instamart testing couldn't have caught: items the site reported as automatically added during a run weren't actually landing in the real cart, while items added via the "add anyway" override button did work correctly. Comparing the two code paths found the one structural difference: `agent/workflow.js`'s automatic cart-adds reused one long-lived `InstamartClient` across the *entire* run (already having done `init()`, `get_addresses`, and multiple `search_products` calls before reaching a given `update_cart` call), while `server.js`'s override route always created a brand-new client and called `init()` fresh immediately before its one `update_cart` call -- the exact path confirmed to work.
...
- **Time spent:** ~40 min
- **Rough tokens used:** ~10-12K
- **Context:** the downloadable recipe was plain Markdown (`agent/documentBuilder.js`'s `buildDocument`, saved as `.md`) -- functional, but not a "designed" document. User asked for options; picked styled HTML rendered to a real PDF via a headless browser, reusing the site's own fonts/colors, over the alternatives (plain HTML file, nicer Markdown, or hand-drawing a PDF with a low-level library like pdf-lib).
- **What shipped:** new `agent/pdfBuilder.js` builds a self-contained styled HTML document (inline CSS, `@font-face` rules pointing at the site's already-vendored local font files via `file://` URLs so Puppeteer doesn't need a running HTTP server to load them) reusing the same navy/paper tokens and title/body fonts as the live site, then renders it to a real PDF via `puppeteer` (`page.setContent` + `page.pdf`) and saves it to `output/<slug>-<timestamp>.pdf`. Extracted the ingredient-categorization logic (`onHand`/`substitutions`/`purchases`/`unresolved`) that used to be duplicated between the old Markdown builder and `buildSummary()` into a shared `categorizeDecisions()` in `agent/documentBuilder.js`, which is now a small shared helper module (`slugify` + `categorizeDecisions`) rather than the old Markdown-generation file -- the old `buildDocument`/`saveDocument` functions were deleted outright since nothing called them anymore (confirmed via a repo-wide grep before deleting) rather than left as dead code. `agent/workflow.js` now calls `buildDocumentHtml` → `renderPdf` → `saveDocumentPdf` instead of the old Markdown pair; `server.js`/`public/app.js`'s download link needed zero changes since both already just link to `/output/<file>` generically by whatever extension the saved file actually has.
- **What didn't work on the first try / real testing done:** `npm install puppeteer` alone did not download the actual Chromium binary in this sandboxed environment -- its postinstall script needed explicit approval (`npm approve-scripts puppeteer`) and then had to be run directly (`node node_modules/puppeteer/install.mjs`) before `puppeteer.launch()` would actually work; caught immediately by trying to render a real PDF first rather than assuming `npm install` alone was sufficient. Ran a direct `buildDocumentHtml`/`renderPdf`/`saveDocumentPdf` test producing a real file and confirmed its raw bytes start with the actual `%PDF-1.4` magic header (not just "didn't throw"), then ran the *entire* `agent/workflow.js` end-to-end (stubbed Instamart, real Groq calls, real PDF generation) via a throwaway script and confirmed a genuine multi-section PDF comes out the other end of a real run, not just the isolated builder.
- **Real, flagged, unresolved risk:** Puppeteer bundles/needs a full Chromium at runtime, and this project deploys to Railway via its default Nixpacks Node build (no `Dockerfile`/`nixpacks.toml` in the repo) -- a commonly-hit gotcha where the deployed container may be missing system shared libraries Chromium needs, even though this works fine in local dev. This has only been verified locally; the next real Railway deploy needs to be checked to confirm PDF generation actually works in production. If it doesn't, the documented fallback is a `nixpacks.toml` adding Chromium's apt dependencies, or switching to `puppeteer-core` + a serverless-friendly Chromium build (`@sparticuz/chromium`).

## 2026-09-24 -- Real bug, round 2: update_cart was likely clobbering earlier items, not just using a stale session (same branch)

- **Time spent:** ~30 min
- **Rough tokens used:** ~8-9K
- **Context:** the fresh-client-per-mutation fix from the previous entry was necessary but not sufficient -- the user retested with a real recipe and confirmed, by checking the actual Swiggy cart *before* clicking any override, that only 1 of 3 automatically-"added" ingredients had genuinely landed. Since every automatic add already used a fresh client/session by this point (matching the override route exactly), the "stale reused session" theory couldn't be the (whole) explanation anymore.
- **What shipped:** re-examined the one remaining structural fact about `update_cart`: its name and its `items: [...]` array parameter are exactly the shape of a "set the cart's contents to this list" call, not an "add one item" call -- a common pattern in cart APIs. Under that reading, each automatic add in the per-ingredient loop -- which only ever sent `items: [{ that one product }]` -- would silently overwrite whatever the previous `update_cart` call in the same run had just put in the cart, leaving only the *last* ingredient's item actually surviving. That fits "only 1 of 3 survived" far better than session staleness. Rewrote `agent/cartActions.js`'s `addToCart`/`removeFromCart` to call `get_cart` first, parse its (undocumented, best-effort-parsed) current item list, merge the new item into it (or drop it, for removal), and send the *full* merged list to `update_cart` every time -- so each call is a complete, idempotent statement of "the cart should contain these items" rather than a partial one that can clobber prior state. This is safe even if `update_cart` turns out to actually be a true incremental upsert: re-sending an unchanged existing item at the same quantity is a no-op either way, so this can't make things worse than before.
- **What didn't work on the first try / real testing done:** `get_cart`'s actual response shape has never been observed against a live account in this codebase (Swiggy's Instamart MCP tools aren't consistent about shape -- `get_addresses` uses `structuredContent`, `search_products` buries JSON in `content[1].text`), so `parseCartItems()` tries several plausible shapes and fails open to an empty list (which just reverts to the previous, already-partially-working behavior) rather than throwing if none match. Built a throwaway Node script that stubs a fake Swiggy server implementing genuine replace-semantics for `update_cart`, ran 3 sequential `addToCart` calls followed by a `removeFromCart` through the *real* `agent/cartActions.js` code, and confirmed all 3 items survive together and removal only drops the intended one -- proof the merge logic itself is correct, though it still can't prove this is the actual root cause on Swiggy's live server without the user retesting against their real account.
